...
Сервис поддерживает два режима работы с моделью: `mock` использует встроенную `MockLLM`, а `real` подключает внешнюю LLM через OpenRouter. Режим выбирается в `.env` через `LLM_MODE`. Для mock-режима достаточно `LLM_MODE=mock`; для real-режима нужно указать `LLM_MODE=real`, `LLM_PROVIDER=openrouter`, `LLM_API_KEY=<your_key>`, `LLM_MODEL=openrouter/free`, `LLM_BASE_URL=https://openrouter.ai/api/v1`. 
